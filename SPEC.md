...
| 5 | HTTP/2 フロー制御ウィンドウ未調整 | 初期ウィンドウ 64 KB。高 BDP 環境で帯域の数 % しか使えない | 生 TCP。フロー制御は TCP のみ。`SO_RCVBUF` を明示設定可能 |
...
`sz` feature を有効にしたビルドが `abs_error_bound` を SZ3 で適用する。圧縮アルゴリズムは自前で実装せず外部ライブラリに委ね、`Codec` の値 1 つと `match` の腕 1 つで ZFP など他のアルゴリズムを足せる形にしてある。適用するのは float32 / float64 のみで、それ以外の dtype と、`rel_error_bound`・`DTYPE_CAST`・`SUBSAMPLE` は EXACT にフォールバックする。
...
**ブロック**: 1 つの `DATA` フレームのペイロードが、独立した 1 ブロックである。ブロック単体で展開できるので、順不同の到着・接続間のワークスティーリング・失われたチャンクの再取得が、無損失のときとまったく同じに成立する。ペイロードの先頭 24 バイトは AEX 自身のブロックヘッダで、要素型・ブロックの形状 (高々 3 軸)・誤差上限を持つ。受信側は転送の文脈を何も持たずに展開でき、ZFP のようにストリームが形状を含まないコーデックも同じ経路に載る。
...
圧縮した転送は線ではなく CPU で律速されるため、RTT を振ってもスループットがほとんど動かない。ただし credit は**論理**バイトで数えるので、r 倍に圧縮すると実際に線に乗る in-flight バイトは r 分の 1 になり、遅延の大きい回線ではその分だけ目減りする (RTT 50 ms、8.7 倍圧縮で 15 %)。観測した圧縮率で credit を伸ばすのが自然な次の一手である。
...
message PrepareSelectionRequest {
    ...
    uint32 requested_codec = 6;       // 0 = RAW
}
...
これらは設定ファイルと環境変数で上書きでき、ベンチマークで掃引できるようにする。
...
**解決**: バックエンドが伸長結果を LRU キャッシュに保持する。どの接続が要求しても伸長は 1 回で済み、**クライアントの分割方法から独立に効率が保たれる**。転送チャンク境界をストレージ境界に揃える必要も、サーバが境界をクライアントに伝える必要もなくなる。
...
4.83 〜 72.7 倍。**勝ち負けを決めるのは帯域であって遅延ではない**: 圧縮側は
515 〜 1,554 MiB/s で帯域にも RTT にもほぼ依存せず (CPU 律速)、無損失がそれより遅くなる
回線、すなわち 5 〜 13 Gbit/s から下で勝つ。1 Gbit/s では 4.6 〜 13.7 倍。26 Gbit/s の
VM 間では RTT 50 ms でも無損失が勝つ。
...
| ZFP ほかの誤差上限付きアルゴリズム | SZ3 で先に測る。libpressio の Rust バインディングは未公開で、今は使えない | `Codec` の値 1 つと `codec::compress` / `decompress_into` の腕 1 つ | SZ3 の測定で物足りないと分かった時点 |
...
| `.npy` のみでは研究の主張が弱い | 転送性能はフォーマット非依存であることを示し、M6 以降で Zarr を追加して汎用性を実証する |
