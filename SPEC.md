...
```
逐次:  [pread 4MiB][write 4MiB][pread 4MiB][write 4MiB]  ← 片方が常に待つ
並行:  [pread][pread][pread]...
              [write][write][write]...                    ← 両方が同時に走る
```

データが RAM に収まらない場合 (本基盤の本来の想定) は、この差が支配的になる。接続ごとに**読みスレッドと送りスレッドを分け、有界チャネル (深さ 2〜3 のバッファ) で繋ぐ**。バッファは循環利用し、転送中のアロケーションをゼロにする。
...
tokio の本来の利点は「読みと送りを重ねられる」ことだが、それは 6.5.3 のスレッド分割で同等に達成できる。想定規模が「少数クライアント」であり、スレッド数は `クライアント数 × streams × 2` (数十〜百本程度) に収まるため、**接続ごとに専用 OS スレッド + 同期 I/O** が最も単純で、性能上の損もない。コントロールプレーンのみ tokio で動かす。
...
**適応品質を実装する際の設計メモ**: `SUBSAMPLE` と `DTYPE_CAST` はサーバ側で変換が必要になるため、読みバッファとは別に変換先バッファが要り、変換コストが加わる。ただし転送量が減るため WAN では総合的に勝つ。この「変換コスト vs 転送量削減」のトレードオフをどこで切り替えるかが研究的な論点になる。`QualitySpec` をクライアントが明示指定する形から始め、自動判断は測定結果を踏まえて後から載せる。
