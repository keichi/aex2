...
圧縮した転送は線ではなく CPU で律速されるため、RTT を振ってもスループットがほとんど動かない。ただし credit は**論理**バイトで数えるので、r 倍に圧縮すると実際に線に乗る in-flight バイトは r 分の 1 になり、遅延の大きい回線ではその分だけ目減りする (RTT 50 ms、8.7 倍圧縮で 17 %)。観測した圧縮率で credit を伸ばすのが自然な次の一手である。
