...
**コーデック**は現状 `bytes` (リトルエンディアンのみ) と恒等の `transpose` だけを
受け付け、それ以外は open 時に理由を付けて拒否する。**zarr-python の既定圧縮器は
zstd なので、既定で書かれたストアはまだ開けない。** 非圧縮のチャンクもデコード
キャッシュを通す。経路が 1 本になり、`decoded_chunk_bytes` が常に値を返すので
サーバのキャッシュ不足警告もそのまま効く。
...
| Zarr の blosc コーデックと入れ子 shard | 評価用ストアは自分で作れる。blosc-src は cmake を要求する | コーデック連鎖の腕を 1 つ | 実データで当たった時点 |
...
| `.npy` のみでは研究の主張が弱い | Zarr v3 を追加済み (第 7.6 節)。チャンク 1 個 = ファイル 1 個という別種のストレージ構造で、転送経路の結論が変わらないことを測る |
