...
fn a_session_reports_what_the_server_granted() {
    ...
    // Bit 0 is RAW and EXACT, always. Bit 3 is SZ and bit 4 ZFP, each only in
    // a build that has it; ERROR_BOUND rides on bit 3 of the encodings as soon
    // as there is any codec to carry it.
    let sz = cfg!(feature = "sz") as u32;
    let zfp = cfg!(feature = "zfp") as u32;
    assert_eq!(session.supported_codecs, 1 | sz << 3 | zfp << 4);
    assert_eq!(session.supported_encodings, 1 | (sz | zfp) << 3);
