...
fn a_session_reports_what_the_server_granted() {
    ...
    // Bit 0 is RAW and EXACT, always. Bit 1 is SZ and bit 2 ZFP, each only in
    // a build that has it; ERROR_BOUND rides on bit 2 of the encodings as soon
    // as there is any codec to carry it.
    let sz = cfg!(feature = "sz") as u32;
    let zfp = cfg!(feature = "zfp") as u32;
    assert_eq!(session.supported_codecs, 1 | sz << 1 | zfp << 2);
